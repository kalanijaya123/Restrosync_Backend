    ...
    public static MenuItem toEntity(MenuItemCreateDto dto) {
        return new MenuItem(
                ...
                dto.recipe() != null ? dto.recipe().stream()
                        .map(r -> new MenuItem.RecipeItem(r.ingredientId(), r.ingredientName(), r.quantities()))
                        .toList() : null,
                dto.extras() != null ? dto.extras().stream()
                        .map(e -> new MenuItem.ExtraItem(e.id(), e.name(), e.price(), e.quantityPerUnit(),
                                e.ingredientId()))
                        .toList() : null);
    }
    ...
    public static MenuItemResponseDto toResponseDto(MenuItem item) {
        return MenuItemResponseDto.builder()
                ...
                .recipe(item.recipe() != null ? item.recipe().stream()
                        .map(r -> new MenuItemResponseDto.RecipeItemDto(r.ingredientId(), r.ingredientName(),
                                r.quantities()))
                        .toList() : null)
                .extras(item.extras() != null ? item.extras().stream()
                        .map(e -> new MenuItemResponseDto.ExtraItemDto(e.id(), e.name(), e.price(),
